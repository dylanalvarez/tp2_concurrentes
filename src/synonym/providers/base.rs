...
use crate::synonym::providers::{merriam_webster, thesaurus, thesaurus2};
...
pub fn synonyms(
    word: &str,
    provider: &Provider,
    result_builder_sender: Sender<ResultBuilderMessage>,
    max_concurrent_requests_semaphore: &Arc<Semaphore>,
    time_between_requests_has_elapsed_condvar: &(Mutex<bool>, Condvar),
    sleeper_sender: Sender<()>,
) {
    ...
    match fetch_synonyms_raw_response(
        word,
        base_url,
        max_concurrent_requests_semaphore,
        time_between_requests_has_elapsed_condvar,
        sleeper_sender,
    ) {
        Err(error) => {
            panic!("{}", error);
        ...
        Ok(response_body) => {
            match match provider {
                ...
                Provider::MerriamWebster => {
                    merriam_webster::raw_response_to_synonyms(response_body)
                }
                Provider::Thesaurus2 => thesaurus2::raw_response_to_synonyms(response_body),
            } {
                Ok(synonyms) => {
                    for synonym in synonyms {
                        match result_builder_sender.send(NewSynonym {
                            word: word.to_string(),
                            synonym,
                        }) {
                            Ok(_) => {}
...
fn fetch_synonyms_raw_response(
    word: &str,
    base_url: &str,
    max_concurrent_requests_semaphore: &Arc<Semaphore>,
    time_between_requests_has_elapsed_condvar: &(Mutex<bool>, Condvar),
    ...
    let result =
        http_requester::fetch_synonyms_raw_response(word.to_string(), base_url.to_string());
