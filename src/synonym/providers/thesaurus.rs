pub fn raw_response_to_synonyms(raw_response: String) -> Result<Vec<String>, String> {
    ...
    let tmp_synonyms_vec = raw_response
...
impl SingleSynonym {
    pub fn get_synonym(&mut self) -> String {
        let tmp_wrapper = self
            .raw_html
            .split("<div class=\"synonym-link-wrapper\"")
            .collect::<Vec<&str>>()[1];
        let synonym_link_wrapper = tmp_wrapper.split("</div>").collect::<Vec<&str>>()[0];
        let tmp_synonym_link = synonym_link_wrapper
            .split("synonym-link")
            .collect::<Vec<&str>>()[1];

        if tmp_synonym_link.contains("</a>") {
            tmp_synonym_link.split("</a>").collect::<Vec<&str>>()[0]
                .split(">")
                .collect::<Vec<&str>>()[1]
                .to_string()
        } else {
            tmp_synonym_link.split("</span>").collect::<Vec<&str>>()[0]
                .split(">")
                .collect::<Vec<&str>>()[1]
                .to_string()
        }
