...
use crate::{ResultBuilderMessage, synonym};
use crate::ResultBuilderMessage::NoMoreSynonyms;

pub fn without_actors(filename: &String, max_concurrent_requests: usize, min_seconds_between_requests: u64) {
    let max_concurrent_requests_semaphore = Arc::new(Semaphore::new(max_concurrent_requests as isize));
    ...
    for provider in [Thesaurus, Thesaurus2, MerriamWebster].iter() {
        ...
        match file_parser::read_lines(filename) {
                ...
                for word in lines {
                    match word {
                        Ok(word) => {
                            handle_log(
                                &log_sender_provider,
                                format!("[Provider {:?}] Fetching synonyms for word: {:?}", provider, word)
                            );
                        ...
                        Err(error) => { 
                            handle_log(
                                &log_sender_provider,
                                format!("[Provider {:?}] Failed to read lines from file: {:?}", provider, error)
                            );
                            panic!("{}", error) 
                        }
                    };
            ...
            Err(error) => {
                handle_log(
                    &log_sender_provider,
                    format!("[Provider {:?}] Couldn't read words.txt: {}", provider, error)
                );
        ...
        Ok(result) => {
            match result {
                Ok(result) => { 
                    handle_log(
                        &log_sender,
                        format!("[Main] Final result {:?}", result)
                    );
                    println!("{:?}", result) 
                }
